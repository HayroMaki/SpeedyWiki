...
Write-Host "Starting SpeedyWiki API Deployment..." -ForegroundColor Green
...
Write-Host "Triggering App Runner deployment for $SERVICE_API..." -ForegroundColor Yellow

try {
    $json = aws apprunner list-services --region $REGION_APP_RUNNER --output json
    if ($json) {
        $data = $json | ConvertFrom-Json
        $ServiceArn = $data.ServiceSummaryList | Where-Object { $_.ServiceName -eq $SERVICE_API } | Select-Object -ExpandProperty ServiceArn
        ...
        if ($ServiceArn) {
            ...
            Write-Host "   Warning: Service $SERVICE_API not found!" -ForegroundColor Red
        }
    }
} catch {
